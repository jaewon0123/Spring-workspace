...
public class FileUploadUtil {
	public static void saveFile(String uploadDir, String fileName, MultipartFile mpFile) throws IOException {
		...
		Path uploadPath = Paths.get("C:\\C:/Users/user1" + uploadDir);
		...
		/*
		 * 1. 전통적인 try구문(메모리 사용이 아래 코드보다 많음)
		 * try {
		...
		try(InputStream inputStream = mpFile.getInputStream()){
			...
			//  만약에 동일한 이름의 파일이 존재할 경우 덮어쓰기 필수로 작성하는 것이 아님
			
			// 추후에 파일 업로드 할 때 동일한 파일명으로 에러가 발생할 수 있을 경우를 대비하여
			// 작성한 것
			// 선택 1) 덮어쓰기를 해서 에러 안나게 설정
			// 선택 2) 기존에 파일이 존재하면 업로드 못하게 하기
			// 선택 3) 만약에 동일한 파일명이 존재하면 숫자나 업로드날짜를 추가로 작성하기
