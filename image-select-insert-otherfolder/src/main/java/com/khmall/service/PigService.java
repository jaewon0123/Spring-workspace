...
public class PigService {
	...
	/*
	@Autowired 를 사용해서 아래 코드를 모두 작성하지 않아도 됨
	public PigService() {
		this.pigMapper = pigMapper;
	}
	서비스에서 추가적으로 수정된 데이터가 매퍼를 통해 db에 들어가고
	db에서 가져온 sql에 추가적으로 수정을 서비스단계에서 진행한다음 컨트롤러에 전달하거나 전달받기 
	*/
	...
	//public void uploadPig(Pig pig) { -> 돼지정보를 저장할 때 경로나 이름 아무것도 변경할게 없을 때 사용
		public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		// 만약에 파일이름을 저장하길 원한다면 이미지 업로드할 때 임시저장되어있는 이미지 파일이름 가져옴
			...
			if (!폴더경로확인.exists()) {
				...
				// mkdir 써도 되긴 하지만 혹시 모를 문제를 최소화 하기 위해 mkdirs를 사용
			}
