...
public class LoginController {
	...
	public String showMyPage(HttpSession session, Model model) {
		...
		//만약에 로그인이 되어있지 않은데 접속하려한다. 그러면 묻지도말고 바로 홈페이지로 돌려보내자
		if(member == null) {
			return "redirect:/login";
		}
	 ...
	 * Member updateMember 는 소비자(클라이언트)가 새롭게 작성하고 DB에 덮어쓸 내용이 임시저정
	 ...
	 *	
	 */
	@PostMapping("/modifyProfile")//form action에 작성한 주소값 넣기
												// 새롭게 저장할 내용을 담을 공간이름
	public String updateMember(HttpSession session, Member updateMember) {
		...
		// input에 사용자가 작성한 값을 모두 가져오기
		updateMember.setMember_id(member.getMember_id());
		// 기존에 있던 DB에 새롭게 수정한 내용을 덮어쓰기
		memberService.updateMember(updateMember);
		session.setAttribute("loginSession", updateMember);
	 ...
	 * input에서 name , th:field 로 작성된 변수명과 무조건 일치 해야함 @RequestParam("  ")
	 * 
