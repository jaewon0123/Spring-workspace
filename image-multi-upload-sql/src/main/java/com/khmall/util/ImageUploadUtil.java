...
/*
 * html 파일과 DB 가 주고받을 때는 Service 라는 이름으로
 * 코드를 작성하지만
 * 백엔드에서 단독적으로 진행하는 값이나 공통으로 설정할 때는
 * util이라는 이름을 사용해서 코드를 작성
 * 
 * util -> postMan 을 사용해서 파일업로드가 잘 되고 있는 지 테스트 진행
 */
// 이미지업로드서비스 -> 이미지업로드유틸 로 변경
...
public class ImageUploadUtil {
	public static void insertImageUpload(String uploadDir, 
			...
			//컨트롤러에서는MultipartFile[] 배열모양 => 서비스에서 하나씩 올린 파일을 모두 전송
